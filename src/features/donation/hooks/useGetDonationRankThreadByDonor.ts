...
import { getDonationRankThreadByDonor } from '../api/getDonationRankThreadByDonor';
import { useDonationTabStore } from '../state/donationTabStore';

/**
 * ✅ usegetDonationRankThreadByDonor
 * - 특정 수신자(스레드 작성자)의 후원 랭킹 조회
 * - 페이징 + Zustand 캐시 유지
 * - 닫히기 전까지 데이터 유지
 */
export const useGetDonationRankThreadByDonor = (
  threadId: string,
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await getDonationRankThreadByDonor({
        currentMemberId,
        threadId,
      ...
      console.error('❌ getDonationRankThreadByDonor 실패:', err);
    } finally {
      setLoading(false);
    }
